  ...
  describe '.active?' do
    context 'when a port isn\'t supplied' do
      it 'raises an error' do
        expect { AwaitRb::Protocol::TCP.active?(host: '127.0.0.1') }.to raise_error
      end
    end
    ...
    context 'when a host isn\'t supplied' do
      ...
      it 'defaults to `127.0.0.1`' do
        ...
        AwaitRb::Protocol::TCP.active?(port: port)
      end
    end
    ...
    context 'when a service is listening on the port' do
      ...
      subject { AwaitRb::Protocol::TCP.active?(host: host, port: port) }
